...
const Header = ({ isLoggedIn, onLogout }) => {
  ...
  const navList = (
    <ul
      className={`mx-auto max-w-screen-xl px-4 py-2 lg:px-8 lg:py-4 lg:flex lg:gap-5`}
    >
      <Typography
        as={Link}
        to="/"
      ...
      {isLoggedIn ? (
        <div className="flex items-center gap-x-8">
          <Typography className="p-1 font-medium" as={Link} to="/account">
            <svg
              width="25"
              height="25"
              viewBox="0 0 16 17"
          ...
          <Typography
            ...
            variant="small"
            color="blue-gray"
            className="flex items-center gap-x-2 py-2 px-4 font-medium bg-red-600 hover:bg-red-700 text-white"
            onClick={onLogout}
          >
            Logout
          </Typography>
        ...
        <Typography as={Link} className="py-1 px-4 font-medium bg-green-500 hover:bg-green-600 text-white rounded" to="/login">
          Login
        </Typography>
      )}
    </ul>
